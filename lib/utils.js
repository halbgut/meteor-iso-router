...
generateEventParameters = function generateEventParameters (request, response, next) {
  let path = IsoRouter.location(request)
  let route = Object.create(IsoRouter.getRouteForUrl(path))
  return {
    request,
    req: request,
    response,
    ...
    route,
    path,
    parameters: route.matchToObject(getCleanPath(path))
  }
}
